...
import telegram
import sys
sys.path.insert(0, '../TelegramHeartsAPIKey')
...
def start(bot, update):
	...
	if chat.id == user.id:
		return send_private_message("Can only start a game in a group",
			user)
	else:
		if chat.id in Groupchat.active_groups:
			return send_public_message("A game has already been started \
				in this group", user, chat, user_specific = False)
		else:
			Groupchat(chat)
			return send_public_message("Successfully started a game, \
...
def end(bot, update):
	...
	user = update.message.from_user
	if chat.id == user.id:
		return send_private_message("End a game by sending /end in the \
			group chat where the game was created")
	else:
		if chat.id not in GroupChat.active_groups:
			return send_public_message("There is no ongoing game in this \
				group", user, chat)
		else:
			group = GroupChat.active_groups[chat.id]
			group.remove_group_and_active_users()
			return send_public_message("Game ended", user, chat)

def join(bot, update):
	chat = update.message.chat
	user = update.message.from_user
	if chat.id == user.id:
		return send_private_message("Please join the game via a group",
			user)
	else:
		return join_game_public(user, chat)

def join_game_public(user, chat):
	# game not initiated in the group
	if chat.id not in Groupchat.active_groups:
		return send_public_message("Please initiate a hearts \
	...
	if user.id in PrivateChat.user_ids_to_group:
		...
		if group == joined_group:
			return send_public_message("Already joined the game in this \
				group", user, chat)
		else:
			send_public_message("Already joined the game in another group",
				user, chat)
			return send_private_message("Aready joined the game in the \
	...
	if group.match_started or len(group.private_chats) == 4:
		return send_public_message("Game in this group is already full",
			user, chat)
	# game has been initiated, has available spots, user also available
	# user successfully joins game
	return successful_join_game_public(user, group)

def successful_join_game_public(user, group):
	# add user
	group.private_chats.append(PrivateChat(user, group))
	send_public_message("%s %s successfully joined the game"%(
		user.first_name, user.last_name), group.chat)
	send_private_message("Successfully joined the game in %s"%(
		group.title), user.id)
	# start match if full
...
def give(bot, update, args):
	...
	user = update.message.from_user
	if chat.id != user.id:
		return send_public_message("Please give cards in private chat",
			chat)
	if user.id not in PrivateChat.user_ids_to_group:
		return send_private_message("Please join a game first", user)
	joined_group = PrivateChat.user_ids_to_group[user.id]
	if not joined_group.match_started:
		return send_private_message("Match hasn't started yet", user)
	if not joined_group.match.game.game_state != 'passing':
		return send_private_message("Now is not the time to give cards \
			away", user)
	try:
		player_no = find_player_no(user, joined_group.private_chats)
		joined_group.match.game.make_move_passing(player_no, ' '.join(args))
		send_to_all_private("%s has passed his cards"%user.first_name,
			joined_group.private_chats)
		if joined_group.match.game.game_state == 'playing':
			send_to_all_private("All cards have been passed",
				joined_group.private_chats)
			return joined_group.prompt_move()
	except NameError as err:
		return send_private_message(err.args[0], user)
...
def play(bot, update, args):
	...
	user = update.message.from_user
	if chat.id != user.id:
		return send_public_message("Please play in private chat",
			chat)
	if user.id not in PrivateChat.user_ids_to_group:
		return send_private_message("Please join a game first", user)
	joined_group = PrivateChat.user_ids_to_group[user.id]
	if not joined_group.match_started:
		return send_private_message("Match hasn't started yet", user)
	if not joined_group.match.game.game_state != 'playing':
		return send_private_message("Now is not the time to play cards",
			user)
	try:
		player_no = find_player_no(user, joined_group.private_chats)
		res = joined_group.match.game.make_move_playing(player_no,
			' '.join(args))
		send_to_all_private("%s has played the move %s"%(
			user.first_name, args[0]), joined_group.private_chats)
		if len(res) == 0:
			joined_group.prompt_move()
		else:
			send_to_all_private("%s got the cards from the round"%(
				joined_game.private_chats[res[0]].user.first_name),
				joined_group.private_chats)
			if len(res) > 1:
				names, points = (map(lambda x: x.user.first_name,
					joined_group.private_chats), res[1])
				send_to_all_private("The current game finished!\n\
...
def send_public_message(msg, user, chat, user_specific = True):
	pass

def send_private_message(msg, user):
	pass

def send_to_all_private(msg, private_chats):
	pass
...
class GroupChat:
	...
	def __init__(self, chat):
		...
		self.match = None
		GroupChat.active_groups.add(self.chat.id)
	...
	def start_match(self):
		...
		send_to_all_private("Match starting!\nN: %s\nE: %s\nS: %s\nW: %s\n"%(
	...
	def start_game(self):
		...
		for (i, private_chat) in enumerate(self.private_chats):
			send_private_message("Your cards are: %s"%(
				self.match.players[i].current_hand), user)
		# tell them to give away 3 accordingly if needed
		if self.match.game.game_state == 'playing':
			return self.prompt_move()
		else:
			send_to_all_private("Select three cards to pass by sending \
	...
	def prompt_move(self):
		...
		playing_order = [(game.first_player_no + i) for i in range(3)]
		...
		msg = craft_general_prompt_move(cards_on_table, user_to_play,
			round_no, order)
		send_to_all_private(msg, self.private_chats)
		# private msg to next player
		private_msg = "Your turn! Your cards are:\n%s\nEnter /play [card]\
			to play"%(self.match.players[current_player_no].current_hand)
		send_private_message(private_msg, user_to_play)
	def end_match(self):
...
class PrivateChat:
	...
	def __init__(self, user, group):
		...
		Groupchat.user_ids_to_group[user.id] = group
